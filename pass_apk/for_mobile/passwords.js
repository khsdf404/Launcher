$('document').ready(function() {
    ...
    function RecreatePass() {
        ...
        for (i = 0; i < storage_one.length; i++) {
            $('.pass_wrapp .column1').append(`
                <div class="p_case">
                    <div class="p_name"> <a>${storage_one[i].name}</a></div>
        ...
        for (i = 0; i < storage_two.length; i++) {
            $('.pass_wrapp .column2').append(`
                <div class="p_case">
                    <div class="p_name"><a>${storage_two[i].name}</a></div>
                    <div class="p_login">Login</div>
                    <div class="p_password">Password</div>
                </div>
            `);
    ...
    function Encrypt(column, storage, type) {
        for (u=0; u < storage.length; u++) {
            ...
            if (type == 'login') {
                ...
                //$(`.pass_wrapp .column${column}`).find(`.p_case`).eq(u).find(`.p_login`).addClass('blur')
            }
            else {
                $(`.pass_wrapp .column${column}`).find(`.p_case`).eq(u).find(`.p_password`).text( answer);
                //$(`.pass_wrapp .column${column}`).find(`.p_case`).eq(u).find(`.p_password`).addClass('blur')
            }
        }
    ...
    function Shuffle() {
        for (i = 0; i < $(`.column1 a`).length; i++) {
            ...
            $(`.column1 .p_login`).eq(i).shuffleLetters( { "text": $(`.column1 .p_login`).eq(i).text() } );
            $(`.column1 .p_password`).eq(i).shuffleLetters( { "text": $(`.column1 .p_password`).eq(i).text() } );
        }
        for (i = 0; i < $(`.column2 a`).length; i++) {
            $(`.column2 a`).eq(i).shuffleLetters({ "text": $(`.column2 a`).eq(i).text() });
            $(`.column2 .p_login`).eq(i).shuffleLetters( { "text": $(`.column2 .p_login`).eq(i).text() } );
            $(`.column2 .p_password`).eq(i).shuffleLetters( { "text": $(`.column2 .p_password`).eq(i).text() } );
        }
    }
    function PassInput() {
    ...
    function Blur_Pass() {
        $(`.pass_input_div .non_blur`).click(function () {
            $(`.p_login`).hasClass('blur') ?
                $(this).removeClass().addClass(`fa-eye-slash non_blur`) :
                $(this).removeClass().addClass(`fa-eye non_blur`);
            $(`.p_login, .p_password`).toggleClass('blur');
          //  $(`.p_login, .p_password`).toggleClass('opacity');
        });
    }
    ...
    Blur_Pass();
    Show__LoginPass();
    $(`.pass_input_div .non_blur`).trigger('click');
    // EditPassword();
    setTimeout(function () {
      Shuffle();
    }, 3030)
